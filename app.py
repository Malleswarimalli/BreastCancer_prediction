...
# Fallback to local dataset for testing
if uploaded_file is not None:
    data = pd.read_csv(uploaded_file)
else:
    st.write("No file uploaded. Using default `BreastCancer.csv` for testing.")
    try:
        data = pd.read_csv("BreastCancer.csv")
    except FileNotFoundError:
        st.error("`BreastCancer.csv` not found. Please upload a file or provide the dataset.")
        st.stop()
